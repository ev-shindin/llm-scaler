...
var _ = Describe("Estimate", func() {
	...
	It("lets a single reading order while the scheduler holds a queue", func() {
		...
		By("ordering even while a replica is starting, which is what a ramp is")
		// An earlier version withheld the figure whenever a start was
		// outstanding, and run 26 measured it inert: through every cycle of a
		// ramp a deployment has more replicas than are Ready. The engine
		// already declines to re-order supply in flight --
		// RC = max(0, D/scaleUp - anticipated) -- so the floor need not.
		pending := []domain.VariantCapacity{{VariantName: "v", Role: domain.RoleDecode,
			ReplicaCount: 1, PendingReplicas: 1, PerReplicaCapacity: float64(runK1)}}
		h := Estimate(runLambda, one, pending, nil, BacklogDrainSeconds, 0.85, false, runLambda*10)
		Expect(h.Terms[domain.RoleDecode].OrderedBehindQueue).To(BeTrue(),
			"a ramp always has a start outstanding; withholding the figure then is what made this inert")
