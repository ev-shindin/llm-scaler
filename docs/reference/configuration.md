...
| `WVA_QUOTAS` | Required by `WVA_LIMITER=quota`: the per-accelerator budget, as `TYPE=N` pairs (`'H200=8 A100=4'`). `-1` is no cap on that type. No default — an empty quota entry is zero, not unlimited | — |
