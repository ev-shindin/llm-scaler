...
"$PY" - "$T/render.yaml" "$T/render-default.yaml" "$LONG" <<'PYEOF' || fail=1
import re, sys, yaml
...
if bash deploy/prepull.sh apply -n check-ns --image a:1 --node-selector notakeyvalue --dry-run >/dev/null 2>&1; then
    bad "a --node-selector without '=' must be refused"
else
...
EOF
got="$(bash hack/benchmark/engine_image.sh "$T/clone")"
if [ "$got" = "docker.io/vllm/vllm-openai:v0.26.0" ]; then ok "engine_image.sh reads repository:tag with the anchor resolved"; else bad "engine_image.sh printed '$got'"; fi
if out="$(bash hack/benchmark/engine_image.sh "$T/nonexistent" 2>/dev/null)"; then bad "engine_image.sh must fail on a missing clone"; else
    [ -z "$out" ] && ok "engine_image.sh prints nothing and fails on a missing clone" || bad "engine_image.sh printed on failure: '$out'"; fi
mkdir -p "$T/clone2/config/templates/values"
printf 'images:\n  vllm:\n    repository: docker.io/vllm/vllm-openai\n' > "$T/clone2/config/templates/values/defaults.yaml"
if bash hack/benchmark/engine_image.sh "$T/clone2" >/dev/null 2>&1; then bad "engine_image.sh must fail without a tag"; else ok "engine_image.sh fails without a tag rather than guessing"; fi

# ---------------------------------------------------------------------------
# 3. status, offline, against what a cluster answered.
# ---------------------------------------------------------------------------
IMG=docker.io/vllm/vllm-openai:v0.26.0
bash deploy/prepull.sh apply -n check-ns --image "$IMG" --dry-run > "$T/one.yaml"
DS="$(grep -m1 '^  name:' "$T/one.yaml" | awk '{print $2}')"
...
cat > "$T/kubectl" <<EOF
#!/usr/bin/env bash
# stub: answers the three reads status makes, from the fixtures
case "\$1 \$2" in
  "get nodes")     cat "$T/nodes.json" ;;
  "get pods")      cat "$T/pods.json" ;;
  "get daemonset") printf '%s\n' "${IMG}" ;;
...
set +e
KUBECTL="$T/kubectl" bash deploy/prepull.sh status -n check-ns > "$T/status.out" 2>&1
rc=$?
set -e
expect_line() {  # node, IMAGE column
    if grep -qE "^  $1 +$2 " "$T/status.out"; then ok "status: $1 -> $2"; else bad "status: $1 expected $2; got: $(grep -E "^  $1 " "$T/status.out" || echo none)"; fi
}
...
grep -q 'node-evicted .*Evicted' "$T/status.out" && ok "status: the evicted holder shows its reason" || bad "status: Evicted reason missing"
grep -q 'node-pulled .*not held' "$T/status.out" && ok "status: a pulled-but-unheld image says so" || bad "status: pulled/not-held note missing"
grep -q '3/6 nodes hold it; 3 do not' "$T/status.out" && ok "status: the tally counts pulled as held and the rest as not" || bad "status: tally wrong: $(grep 'nodes hold' "$T/status.out")"
[ "$rc" -ne 0 ] && ok "status: exits non-zero while a node lacks the image" || bad "status: exit 0 with nodes lacking the image"
# the image discovery path (no --image) used the daemonset stub: the header names it
grep -q "^${IMG}$" "$T/status.out" && ok "status: with no --image the held images are discovered from the DaemonSets" || bad "status: discovery from DaemonSets failed"

# no node matches: verdict mode exits, report mode (after apply) warns and continues
printf '{"items":[]}' > "$T/nodes.json"
if KUBECTL="$T/kubectl" bash deploy/prepull.sh status -n check-ns --image "$IMG" >/dev/null 2>&1; then bad "status with no matching node must fail"; else ok "status: no matching node is an error"; fi
cat > "$T/kubectl-apply" <<EOF
#!/usr/bin/env bash
case "\$1 \$2" in
  "apply -n")      cat >/dev/null; echo "daemonset.apps/x configured" ;;
  "get nodes")     cat "$T/nodes.json" ;;
  "get pods")      cat "$T/pods.json" ;;
  *) echo "stub kubectl: unexpected \$*" >&2; exit 1 ;;
esac
EOF
chmod +x "$T/kubectl-apply"
if KUBECTL="$T/kubectl-apply" bash deploy/prepull.sh apply -n check-ns --image "$IMG" > "$T/apply.out" 2>&1; then
    grep -q 'will run nowhere' "$T/apply.out" && ok "apply: a selector matching no node warns and still succeeds" || bad "apply: the no-node warning is missing"
else
    bad "apply must not fail because no node matched the selector (log_error inside status ended the process)"
fi

# ---------------------------------------------------------------------------
# 4. The Makefile's comma list, and the standup's composition of it.
# ---------------------------------------------------------------------------
line="$(make -n prepull IMAGES=a:1,b:2 NAMESPACE=ns 2>/dev/null | grep 'prepull.sh apply')"
case "$line" in
    *"--image a:1 --image b:2"*) ok "make prepull: IMAGES=a,b becomes --image a --image b" ;;
    *) bad "make prepull expanded to: $line" ;;
esac
line="$(make -n prepull-delete NAMESPACE=ns 2>/dev/null | grep 'prepull.sh delete')"
case "$line" in *"--all"*) ok "make prepull-delete with no IMAGES deletes every holder" ;; *) bad "prepull-delete: $line" ;; esac
composed="$(printf '%s' "a:1,b:2" | tr ',' '\n' | sed 's/^/--image /' | tr '\n' ' ')"
[ "$composed" = "--image a:1 --image b:2" ] && ok "standup: the tr/sed composition yields --image a --image b" || bad "standup composition: '$composed'"

# ---------------------------------------------------------------------------
# 5. The scenarios: engine containers pull IfNotPresent, the harness's init
#    container is left as it was.
# ---------------------------------------------------------------------------
"$PY" - hack/benchmark/scenarios/guides/*.yaml <<'PYEOF' || fail=1
import sys, yaml
bad = []
