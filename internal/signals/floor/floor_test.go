...
var _ = Describe("Estimate", func() {
	...
	It("sizes the fleet to lambda over mu, in the variant's own tokens", func() {
		...
		f := Estimate(runLambda, replicas(6), variants(domain.RoleDecode, 6), nil, BacklogDrainSeconds, 0.85)
	...
	It("does not move when replicas are added or removed", func() {
		...
		// replica as at six.
		one := Estimate(runLambda, replicas(1), variants(domain.RoleDecode, 1), nil, BacklogDrainSeconds, 0.85)
		six := Estimate(runLambda, replicas(6), variants(domain.RoleDecode, 6), nil, BacklogDrainSeconds, 0.85)
	...
	It("asks for the replicas the load needs, whether the fleet has them or not", func() {
		...
		f := Estimate(runLambda, replicas(1), variants(domain.RoleDecode, 1), nil, BacklogDrainSeconds, 0.85)
		Expect(f.ByRole[domain.RoleDecode]).To(BeNumerically("~", runLambda/runMu*float64(runK1), 1e-6))
		...
		g := Estimate(runLambda, replicas(2), variants(domain.RoleDecode, 2), nil, BacklogDrainSeconds, 0.85)
	...
	It("holds but does not order on a single reading", func() {
		...
		pend := Estimate(100, one, pending, nil, BacklogDrainSeconds, 0.85)
		...
		z := Estimate(runLambda, one, variants(domain.RoleDecode, 1), nil, BacklogDrainSeconds, 0)
	...
	It("prices a backlog as arrivals to clear within the drain target", func() {
		...
		f := Estimate(runLambda, replicas(1), variants(domain.RoleDecode, 1), backlog, 60, 0.85)
		...
		g := Estimate(runLambda, replicas(1), variants(domain.RoleDecode, 1), backlog, 120, 0.85)
		...
		z := Estimate(runLambda, replicas(1), variants(domain.RoleDecode, 1), backlog, 0, 0.85)
	...
	It("says nothing without an arrival rate", func() {
		f := Estimate(0, replicas(2), variants(domain.RoleBoth, 2), nil, BacklogDrainSeconds, 0.85)
		Expect(f.ByRole).To(BeEmpty())
	})
})
...
var _ = Describe("Estimate with mixed readings", func() {
	...
	It("takes the median cost across a role's replicas, not the mean or an extreme", func() {
		...
		f := Estimate(6, replicas, variants, nil, BacklogDrainSeconds, 0.85)
		...
		f = Estimate(6, replicas[1:], variants, nil, BacklogDrainSeconds, 0.85)
